...
class summerize_endpoint(APIView):
    permission_classes = (AllowAny,)

    def get(self, request):
        model = Summarizer()
        result = model(request.GET['text'],
                       min_length=50, max_length=int(request.GET['size']))
        summery = ''.join(result)

        result = model(request.GET['text'],
                       min_length=1, max_length=30)
        extractive_headline = ''.join(result)

        abstractive_headline = generate_summary(summery)

        # Response
        return Response({
            'msg': 'success',
            'summery': summery,
            'extractive_headline': extractive_headline,
            'abstractive_headline': abstractive_headline,
